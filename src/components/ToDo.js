  ...
  render() {
    ...
    return (
      <Container fluid={true}>
        ...
        <Row className="mt-2">
          <Col>
          {this.state.tasks.length?<Button
              variant="danger"
        ...
        <Row className="mt-2">
          <Col>
          {this.state.tasks.length?<Button variant="secondary" onClick={this.markOrUnmarkAll} disabled={!this.state.tasks.length}>
              {this.state.tasks.length === this.state.checkedTasks.size? "Remove All":"Check All"}
            </Button>:""}
          </Col>
        </Row>
      </Container>
